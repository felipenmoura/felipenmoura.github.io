...
//	UPDATE EM DADOS PESSOAIS
	...
	$qr= "	   UPDATE tb_dados_pessoais
			  ...
			  ;
					  ...
					  SET
							...
							dt_nascimento= TO_DATE('".trim($_POST['ger_contatos_clienteNasc'])."','DD/MM/YYYY'),
							s_conjuge= '".trim($_POST['conjugeName'])."',
							fk_estado_civil= ".trim($_POST['ger_contatos_estado_civil_cliente'])."
	...
	?>
		<script> try{top.filho.atualizaLista('list_contatos','ger_contatos_list.php');}catch(error){}</script>
			...
			<tr>
				<td colspan="4"
					...
					<textarea name="clienteObs"
							  ...
							  style="width: 100%;
									 height: 150px;"><?php echo htmlentities($linha_dados_pessoais['txt_obs']) ?></textarea>
				</td>
			</tr>
			<tr>
				<td colspan="4"
					...
					<input type="button"
						   class="botao"
						   value="Limpar"
						   onclick="top.setLoad(true);onlyEvalAjax('ger_contato_add.php', '', 'document.getElementById(\'corpo_ger_contato_add\').innerHTML=ajax')">
					<input type="button"
						   class="botaoExcluir"
						   value=""
						   onclick="if(confirm('Tem certeza que deseja excluir este Contato ?'))
													  {
														top.setLoad(true);
														onlyEvalAjax('ger_contato_del.php?delContato=<? echo $_GET['pk_dados_pessoais']; ?>', '', 'eval(ajax)');
													  }">
				</td>
			</tr>
		</table>
