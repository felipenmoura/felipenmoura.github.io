...
', '', $insert)))."')</script>";
					 ...
					 echo "----------------------------------------------------------------";
					 echo "zzz---- ".count($_POST['cliente_contatos'])."<br>";
...
', '', $insert)))."')</script>";
		...
		if($data)
		{
			?> 
				<script>
					top.showAlert('informativo','Filial cadastrada');
					top.removeBlockKey();
					//top.filho.closeBlock('agenda_contato_filial');
				</script>
			<?php
		}else{
			?> 
...
<div style="width: 100%;
	...
	<form name="form_cliente_add" 
		  ...
		  action="agenda_contato_filial.php"
		  method="POST">
	<div>
									...
									<tbody id="<?php echo $PREID; ?>contatoList">
										<tr>
											<td style="padding-right: 3px;">
												<?php
													$_GET['component']		= 'explorer';
													$_GET['componentId']	= $PREID.'clienteAdd_contatoDaEmpresa';
													$_GET['componentValue']	= '';
			...
			<tr>
				<td colspan="4"
					...
					<input type="button"
						   class="botao"
						   value="Cancelar"
						   onclick="closeBlock(top.c.document.getElementById('agenda_contato_filial'));">
					-->
				</td>
			</tr>
		</table>
