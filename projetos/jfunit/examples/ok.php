...
// testing the sum
	...
	jfUnit.assert(sum, 4, 4, 8);
	jfUnit.assert(sum, 4, 2, 6);

	// testing the getStr
	jfUnit.assert(getEmail, 'felipenmoura', 'gmail.com', 'felipenmoura@gmail.com');
	jfUnit.assert(getEmail, 'anotheremail', 'domain.com', 'anotheremail@domain.com');

	// testing booleans
	jfUnit.assert(getBool, true);
	jfUnit.assertNot(getBool, false);

	// using an object as conf
	jfUnit.assert({
		call:sum, // required
		// any other propertie not required or opitional will be treated as
		// parameters to sent in the specified order
		value1:2,
		value2:5,
		expected:7 // required
	});
