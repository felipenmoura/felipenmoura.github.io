...
include "inc/class_explorer.php";
if(!$db_connection)
{
...
if($_POST)
{
	echo $_POST['clienteAdd_responsavel'];
	if(!$db_connection)
	{
		include("../connections/flp_db_connection.php");
...
//	UPDATE EM DADOS PESSOAIS
	...
	$qr= "     UPDATE tb_dados_pessoais
		...
		if(!$data)
		{
			echo"<script>alert('Falha na tentativa de Atualização - Filial')</script>";
...
//                                                   INSERINDO EMAILS
		...
		?>
		<script>try{top.filho.atualizaListaContato('list_filiais','ger_filiais_list.php');}catch(error){};</script>
	<?php
	exit;
}
...
//       SELECIONA CONTATOS  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	$select_pess_juridica_cont = "    select vfk_contato,
									   ...
									   where ce.fk_dados_pessoais = dp.pk_dados_pessoais
										 and dp.pk_dados_pessoais = ".$_GET['pk_contato']."
							";
...
?>
<iframe id="<?php echo $PREID; ?>hiddenFrameFilialtAdd"
		name="hiddenFrameFilialAdd"
		style="display: none;">
</iframe>
<div id="<?php echo $PREID; ?>filialAddInvisibleDiv"
	...
	<form name="<?php echo $PREID; ?>form_cliente_add" 
		  target="hiddenFrameFilialAdd"
		  id="<?php echo $PREID; ?>form_cliente_add"
		  action="ger_filiais_edit.php"
		  method="POST">
		<input type="hidden"
		...
		<table width="100%" id="<?php echo $PREID; ?>tbodyClienteJuridico">
			<tr>
				<td style="width:150px;">
					...
					<input type="text"
						   ...
						   oldValue="<?php echo $linha_pess_juridica['razao_social'] ?>"
			...
			<tr>
				...
				<td style="width:150px;">
					...
					<?php
						//echo htmlentities($linha_pess_juridica_resp['nome']);
			...
			<tr>
				<td colspan="4"
					...
					<textarea class="discret_input"
							  ...
							  style="width: 100%;
									 height: 70px;
									 background-color:white"><? echo htmlentities($linha_dados_pessoais['txt_obs']); ?></textarea>
				</td>
			</tr>
			<tr>
				<td colspan="4"
					...
					<input type="button"
						   class="botao"
						   value="Novo"
						   onclick="top.setLoad(true);onlyEvalAjax('ger_filial_add.php', '', 'document.getElementById(\'corpo_ger_filial_add\').innerHTML=ajax')">
					<input type="button"
						   class="botaoExcluir"
						   value=""
						   onclick="if(confirm('Tem certeza que deseja excluir esta Filial ?'))
								    {
										top.setLoad(true);
										onlyEvalAjax('ger_filial_del.php?delFilial=<? echo $_GET['pk_dados_pessoais'];?>', '', 'eval(ajax)');
								    }">
				</td>
			</tr>
		</table>
